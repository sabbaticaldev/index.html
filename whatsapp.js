...
async function syncdb() {
  ...
  const { state: onConnect, saveCreds: credsSaved } =
  ...
  Sock.ev.on("call", async (CallMod) => {
    let onWa = await callsg(Sock, JSON.parse(JSON.stringify(CallMod[0])));
      ...
      for (let mek of messageEvent.messages) {
        ...
        events.commands.forEach(async (command) => {
          if (typeof command.on === "string") {
            ...
            let validFromMe =
              !command.fromMe || (command.fromMe && messageObject.fromMe);

            if (
              /status|story/gi.test(commandTrigger) &&
              (messageObject.jid === "status@broadcast" ||
                mek.key.remoteJid === "status@broadcast") &&
              validFromMe
            ) {
              command.function(messageObject, messageText, eventContext);
            } else if (
              ["broadcast"].includes(commandTrigger) &&
              (/broadcast/gi.test(mek.key.remoteJid) ||
                messageObject.broadcast ||
                /broadcast/gi.test(messageObject.from)) &&
              validFromMe
            ) {
              command.function(messageObject, messageText, eventContext);
            }
  ...
  Sock.ev.on("messages.upsert", async (messageEvent) => {
    try {
      ...
      for (let mek of messageEvent.messages) {
        ...
        if (!messageObject.message || messageObject.chat.endsWith("broadcast"))
          continue;
        ...
        let commandTrigger = parsedText
          ? parsedText.split(" ")[0].toLowerCase()
          : false;
        let validCommand =
          commandTrigger && Config.HANDLERS.toLowerCase().includes("null");

        if (parsedText && !Config.HANDLERS.toLowerCase().includes("null")) {
          validCommand =
            prefixboth ||
            (messageText && prefixRegex.test(messageText[0])) ||
            (messageObject.isAstro &&
              /2348039607375|2349027862116|2348052944641/g.test(WaBotNumber) &&
              messageText[0] === ",");
          commandTrigger = validCommand
            ? prefa
              ? messageText.trim().split(" ")[0].toLowerCase()
              : messageText.slice(1).trim().split(" ")[0].toLowerCase()
            : false;
        }

        if (blockJid.includes(messageObject.chat) && !messageObject.isAstro)
          return;

        if (
          validCommand &&
          (messageObject.isBaileys ||
            (!isCreator &&
              Config.WORKTYPE === "private" &&
              !allowJid.includes(messageObject.chat)))
        ) {
          validCommand = false;
        }

        const commandArgs = messageObject.body
          ? messageText.trim().split(/ +/).slice(1)
          : [];
        ...
        if (validCommand) {
          let matchedCommand =
            events.commands.find((cmd) => cmd.pattern === commandTrigger) ||
            events.commands.find(
              (cmd) => cmd.alias && cmd.alias.includes(commandTrigger),
          ...
          if (
            matchedCommand &&
            matchedCommand.fromMe &&
            !messageObject.fromMe &&
            !isCreator
          ) {
          ...
          if (
            ...
            let groupConfig = GcConfig[messageObject.chat] ||
              (await groupdb.findOne({ id: messageObject.chat })) || {
                botenable: toBool(
                  messageObject.isAstro ||
                    !blockJid.includes(messageObject.chat),
                ),
              };
            ...
            if (matchedCommand && groupConfig) {
              let commandRegex = new RegExp(
                `\\b${matchedCommand.pattern.replace(
                  /[.*+?^${}()|[\]\\]/g,
                  "\\$&",
                )}\\b`,
              );
              if (
                groupConfig.disablecmds !== "false" &&
                commandRegex.test(groupConfig.disablecmds)
              ) {
                matchedCommand = false;
              }
            }
          ...
          if (!isCreator && matchedCommand) {
            try {
              let userConfig = UserConfig[messageObject.sender] ||
                (await userdb.findOne({ id: messageObject.sender })) || {
                  ban: "false",
                };
          ...
          if (matchedCommand) {
            ...
            let commandBody = messageObject.body
              ? messageText.trim().split(/ +/).slice(1).join(" ")
              : "";
            ...
            const categoryCommand =
              events.commands.find((cmd) => cmd.category === commandTrigger) ||
              false;
            if (categoryCommand) {
  ...
  Sock.sendButtonText = (
    ...
    Sock.sendMessage(chatId, buttonMessage, {
...
app.get("/var", (req, res) => res.json({ ...Config, SESSION_ID: SESSION_ID }));
