...
export default {
  views: {
    ...
    'uix-alert': {
      ...
      render: (
        { title, message, rounded, variant, closable, actions, border },
        { html }
      ) => {
        const colorClass = [
          BgColor[variant],
        ...
        return html`
          <div class="alert ${(rounded && 'rounded') || ''} ${colorClass}">
            <div class="flex justify-between items-center">
              ...
              <div>
                ${closable
                  ? html`<uix-button variant="neutral">&times;</uix-button>`
                  : ''}
              </div>
            </div>
            ${actions.length > 0
    ...
    'uix-mockup-code': {
      ...
      render: ({ prefix, code, highlight, variant }, { html }) => {
        const colorSchema = variant
          ? [BgColor[variant], TextColor[variant]].join(' ')
          : '';
    ...
    'uix-block': {
      ...
      render: (props, { html }) => {
        ...
        return html`
          <div
            class="${spacingClass} ${variantClass} ${bgClass} ${textClass} ${roundedClass} ${shadowClass} ${props.class}"
          >
            <slot></slot>
          </div>
        `;
    ...
    'uix-menu-item': {
      ...
      render: (
        { icon, label, click, href, active, classes = {} },
        { html }
      ) => {
    ...
    'uix-menu': {
      ...
      render: (
        ...
        const itemClass = [
          'flex flex-row items-center hover:bg-gray-900 hover:text-gray-100',
          itemsClass,
          (isActive && 'active') || '',
        ]
          .filter((c) => !!c)
    ...
    'uix-progress': {
      ...
      render: ({ value, max, width, variant }, { html }) => {
        ...
        return html`
          <progress
            class="${progressClass} ${variantClass}"
            value="${value}"
            max="${max}"
          ></progress>
        `;
    ...
    'uix-indicator': {
      ...
      props: {
        ...
        verticalPosition: {
          type: String,
          defaultValue: 'top',
          enum: Positions.filter((p) =>
            ['top', 'middle', 'bottom'].includes(p)
          ),
        },
        responsivePositions: {
          type: Object,
