...
baseTheme.commonColors = [
  ...
  "violet", "purple", "pink", "rose", "blue-gray",
];
...
const baseVariants = {
  ...
  medium: "medium", semibold: "semibold", bold: "bold", extrabold: "extrabold", black: "black"
};

const baseFonts = { mono: "mono", sans: "sans", serif: "serif" };

const baseGaps = { sm: 2, md: 4, lg: 8, xl: 16, "2xl": 32, "3xl": 64, "4xl": 96 };
...
const baseLeadingSizes = {
  "": "", xs: 3, sm: 4, base: 5, md: 6, lg: 7, xl: 8, "2xl": 9, "3xl": 10,
  "4xl": 10, "5xl": 10, "6xl": 10
};
...
const baseTrackingSizes = { "4xl": "wider", "3xl": "wider", "2xl": "wider", xl: "wide", lg: "wide" };
...
const createProps = (userTheme) => {
  ...
  const ColorPickerClasses = Array.from({ length: 9 }, (_, i) => i + 1).map(
    (shade) =>
      baseTheme.commonColors.map(color => `bg-${color}-${shade}00`).concat(baseTheme.greyColors.map(color => `bg-${color}-${shade}00`)));

  return {
    cls,
    Sizes: ["lg", "md", "sm", "xs", "xl", "2xl", "3xl", "4xl"],
    Variants: ["default", "primary", "secondary", "accent", "neutral", "base", "info", "success", "warning", "error"],
...
export const getElementTheme = (element, props = {}, elementInstance = {}) => {
  ...
  const classes = (typeof defaultElement === "string")
    ? [defaultElement]
    : (typeof defaultElement === "function")
      ? [resolveThemeValue({ elementTheme: defaultElement, props: { ...props, ...elementInstance } })]
      : Object.keys(defaultElement).reduce((acc, attr) => {
        const elementTheme = defaultElement[attr];
        const resolvedThemeValue = resolveThemeValue({ elementTheme, key: elementInstance[attr] });
        if (resolvedThemeValue) acc.push(resolvedThemeValue);
        return acc;
      }, []);
  if (defaultElement["_base"]) classes.push(defaultElement["_base"]);
  if (elementInstance["containerClass"]) classes.push(elementInstance["containerClass"]);
  return classes.join(" ");
};
...
const createBaseTheme = (userTheme, components = {}) => {
  ...
  const generatedTheme = Object.keys(components)
    .map(key => typeof components[key] === "function" ? components[key](theme, props) : components[key])
...
export const extractSafelistFromTheme = (userTheme, components) => {
  ...
  const traverseTheme = obj => {
    ...
    else if (typeof obj === "object" && obj !== null) Object.values(obj).forEach(traverseTheme);
