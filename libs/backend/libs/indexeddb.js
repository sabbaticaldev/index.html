const promisifyRequest = (request) =>
  new Promise((resolve, reject) => {
    if ("onsuccess" in request && "onerror" in request) {
      ...
      request.onerror = () => {
        console.error(
          `IDBTransaction error: ${request.error?.message}`,
          request,
        );
        reject(request.error);
      };
      request.onabort = () => {
        console.error(
          `IDBTransaction abort: ${request.error?.message}`,
          request,
        );
        reject(request.error);
      };
...
const getItem = (key, table) =>
  table("readonly", (store) => promisifyRequest(store.get(key)));
const get = (keys, table) =>
  table("readonly", (store) =>
    Promise.all(keys.map((key) => promisifyRequest(store.get(key)))),
  );
const set = (entries, table) =>
  table("readwrite", (store) => {
...
const remove = (keys, table) =>
  table("readwrite", (store) => {
    keys.forEach((key) => store.delete(key));
    return promisifyRequest(store.transaction);
  });
const update = (key, updater, db) =>
  db("readwrite", (store) =>
